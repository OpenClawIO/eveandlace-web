...
export async function StorefrontPage({ locale }: { locale: Locale }) {
  ...
  const heroProducts = store.products.slice(0, 7).map((product) => localizeProduct(product, locale));
  const categories = store.categories.map((category) => localizeCategory(category, locale));
    ...
    <main>
      ...
      <section className="lower-grid">
        <article className="sync-card" id="sync">
          ...
          <div>
            <h2>{t.syncTitle}</h2>
            <p>{t.lastRefresh}: {formatRefreshTime(store.lastRefreshed, locale)}</p>
            <p>{t.source}: AliExpress Store <a href={store.sourceUrl}>{store.storeId}</a></p>
            <p className="sync-note">{t.syncCopy}</p>
          </div>
          <a className="button button-light" href={store.sourceUrl}>{t.viewSource}</a>
        </article>
      ...
      <footer className="site-footer">
        <div>
          <a className="brand" href={locale === 'en' ? '/' : `/${locale}`}>{store.brand}</a>
          <p>{t.footerTagline}</p>
        </div>
        <div>
          <h3>{t.footerShop}</h3>
